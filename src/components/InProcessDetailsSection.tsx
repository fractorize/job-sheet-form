...
const ProcessDetailForm: React.FC<{
  ...
  const handleMeasurementChange = (field: 'value' | 'unit', value: string | number) => {
  ...
  return (
    <div className="space-y-4 p-4 bg-gray-50 rounded-lg">
      <h4 className="text-sm font-semibold text-gray-800 border-b pb-2">{title}</h4>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="space-y-2">
          <label className="block text-xs font-medium text-gray-700">Operator Sign</label>
          <input
            type="text"
        ...
        <div className="space-y-2">
          <label className="block text-xs font-medium text-gray-700">Machine Number</label>
          <input
            type="text"
        ...
        {showMeasurements && data.measurements && (
          <>
            <div className="space-y-2">
              <label className="block text-xs font-medium text-gray-700">Measurement Value</label>
              <input
                type="number"
                value={data.measurements.value}
                onChange={(e) => handleMeasurementChange('value', parseFloat(e.target.value) || 0)}
                className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"
                placeholder="Value"
                min="0"
                step="0.01"
...
const InProcessDetailsSection: React.FC<InProcessDetailsSectionProps> = ({
  ...
  const handleSkivingChange = (type: 'internal' | 'external', sectionData: ProcessDetail) => {
    onChange({
      skivingDetails: {
        ...data.skivingDetails,
        [type]: sectionData,
    ...
    >
      <div className="space-y-6">
        <ProcessDetailForm
          ...
          onChange={(sectionData) => handleSectionChange('hoseCutDetails', sectionData)}
          showMeasurements={true}
        />

        <div className="space-y-4">
          <h3 className="text-sm font-semibold text-gray-800">Skiving Details</h3>
          <div className="space-y-4">
            <ProcessDetailForm
              title="Internal Skiving"
              data={data.skivingDetails.internal}
              onChange={(sectionData) => handleSkivingChange('internal', sectionData)}
              showMeasurements={true}
            />
            <ProcessDetailForm
              title="External Skiving"
              data={data.skivingDetails.external}
              onChange={(sectionData) => handleSkivingChange('external', sectionData)}
              showMeasurements={true}
            />
          </div>
        </div>
        ...
        <ProcessDetailForm
          ...
          onChange={(sectionData) => handleSectionChange('assemblyDetails', sectionData)}
        />

        <ProcessDetailForm
          title="Mandrals Details"
          data={data.mandralsDetails}
          onChange={(sectionData) => handleSectionChange('mandralsDetails', sectionData)}
        />

        <ProcessDetailForm
          title="Crimping Details"
          data={data.crimpingDetails}
          onChange={(sectionData) => handleSectionChange('crimpingDetails', sectionData)}
          showMeasurements={true}
        />
        ...
        <ProcessDetailForm
          ...
          onChange={(sectionData) => handleSectionChange('weldingDetails', sectionData)}
        />

        <ProcessDetailForm
          title="Punching / Tagging Details"
          data={data.punchingTaggingDetails}
          onChange={(sectionData) => handleSectionChange('punchingTaggingDetails', sectionData)}
        />
      </div>
    </CollapsibleSection>
  );
